...
int sub_count=5;
int n=0,i,j;
char subjects[10][100]={"Tamil","English","Mathematics","Science","Computer_Science"};
...
float calcGrade(int *marks,int sub_count)
{
	float s=0;
	for(i=0;i<sub_count;i++)
	{
...
void showGrades()
{
	...
	for(i=0;i<n;i++)
	{
		avg=calcGrade(s[i].marks,sub_count);
		printf("S.No: %d\n",(i+1));
		printf("---------\n");
		printf(" Student Name:%s\n Student Roll No:%d\n Percentage:%.2f Grade:%s\n---------\n",s[i].name,s[i].rollno,avg,returnGrade(avg));
	}
}
...
int main()
{
   ...
   do
   {
   	...
   	printf("6. Change Subjects\n");
   	printf("7. Show Subjects\n");
   	printf("8. Show Grades\n");
   	printf("9. Exit (All Data will be Freed)\n");
   	printf("-------------------------\n");
   	printf("\nYour Choice:");
   	scanf("%d",&ch);
   	switch(ch)
